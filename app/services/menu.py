...
from app.models.menu.header import (
    MenuHeaderSection, MenuHeaderSectionTranslation,
...
async def get_header_menu(lang_code: str, db: AsyncSession):
    try:
        sections_result = await db.execute(
            select(MenuHeaderSection)
            .where(MenuHeaderSection.is_active == True)
            .order_by(MenuHeaderSection.display_order.asc())
        )
        ...
        for section in sections:
            section_tr_result = await db.execute(
                select(MenuHeaderSectionTranslation).where(
                    MenuHeaderSectionTranslation.section_id == section.id,
                    MenuHeaderSectionTranslation.lang_code == lang_code,
                )
            )
            ...
            items_result = await db.execute(
                ...
                .where(
                    MenuHeaderItem.section_id == section.id,
            ...
            for item in items:
                ...
                sub_items = sub_items_result.scalars().all()

                sub_items_arr = []
                for sub_item in sub_items:
                    sub_item_tr_result = await db.execute(
                        select(MenuHeaderSubItemTranslation).where(
                            MenuHeaderSubItemTranslation.sub_item_id == sub_item.id,
        ...
        return JSONResponse(
            content={"status_code": 200, "data": {"sections": sections_arr}},
...
async def create_header_section(request: CreateHeaderSection, db: AsyncSession):
    try:
        ...
        return JSONResponse(
            content={"status_code": 201, "message": "Header section created.", "id": section.id},
    ...
    try:
        ...
        for lang in LANGS:
            ...
            tr_result = await db.execute(
                select(MenuHeaderSectionTranslation).where(
                    MenuHeaderSectionTranslation.section_id == section_id,
                    MenuHeaderSectionTranslation.lang_code == lang,
                )
            )
            ...
            if tr:
                if label_val is not None:
                    tr.label = label_val
                if base_path_val is not None:
                    tr.base_path = base_path_val
            else:
                db.add(MenuHeaderSectionTranslation(
                    section_id=section_id,
                    lang_code=lang,
                    label=label_val or "",
                    base_path=base_path_val or "",
                ))

        await db.commit()
        return JSONResponse(
            content={"status_code": 200, "message": "Header section updated."},
            status_code=status.HTTP_200_OK,
        )
    except Exception as e:
        logger.exception("500 Internal Server Error")
...
async def delete_header_section(section_id: int, db: AsyncSession):
    try:
        result = await db.execute(
            select(MenuHeaderSection).where(MenuHeaderSection.id == section_id)
        )
        ...
        await db.commit()
        return JSONResponse(
            content={"status_code": 200, "message": "Header section deleted."},
            status_code=status.HTTP_200_OK,
        )
    except Exception as e:
        logger.exception("500 Internal Server Error")
        ...
        if request.title:
            for lang in LANGS:
                ...
                if tr:
                    ...
                    db.add(MenuHeaderItemTranslation(item_id=item_id, lang_code=lang, title=val))
...
async def create_header_sub_item(request: CreateHeaderSubItem, db: AsyncSession):
    try:
        ...
        return JSONResponse(
            content={"status_code": 201, "message": "Header sub-item created.", "id": sub_item.id},
            status_code=status.HTTP_201_CREATED,
        )
    except Exception as e:
        logger.exception("500 Internal Server Error")
...
async def update_header_sub_item(sub_item_id: int, request: UpdateHeaderSubItem, db: AsyncSession):
    try:
        result = await db.execute(
            select(MenuHeaderSubItem).where(MenuHeaderSubItem.id == sub_item_id)
        )
        sub_item = result.scalar_one_or_none()
        if not sub_item:
            return JSONResponse(
        ...
        if request.slug is not None:
            sub_item.slug = request.slug
        if request.display_order is not None:
            sub_item.display_order = request.display_order
        ...
        if request.title:
            for lang in LANGS:
                ...
                if tr:
                    ...
                    db.add(MenuHeaderSubItemTranslation(sub_item_id=sub_item_id, lang_code=lang, title=val))
