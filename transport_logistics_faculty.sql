-- Transport and Logistics Faculty SQL Import Script
...
-- 3. Insert Faculty Directions of Action (Clears old ones to prevent duplicates)
DELETE FROM faculty_directions_of_action WHERE faculty_code = 'transport_logistics';
...
WITH director_insert AS (
    ...
    SET first_name = EXCLUDED.first_name, last_name = EXCLUDED.last_name, email = EXCLUDED.email, phone = EXCLUDED.phone, updated_at = NOW()
    RETURNING id
)
...
-- 6. Insert Director Educations (Clear old ones for clean state)
DELETE FROM faculty_director_educations WHERE director_id IN (SELECT id FROM faculty_directors WHERE faculty_code = 'transport_logistics');
...
-- 7. Insert Deputy Deans (Clear old ones for clean state)
DELETE FROM faculty_deputy_deans WHERE faculty_code = 'transport_logistics';
...
SELECT id, 'az', duty, NOW() FROM (
    ...
    (2, 'Nəqliyyat və logistika fakültəsinin Sosial məsələlər, ictiamyətlə əlaqələr və infrastrukur üzrə dekan müavini')
...
-- 8. Insert Faculty Workers (Academic Advisors) (Clear old ones for clean state)
DELETE FROM faculty_workers WHERE faculty_code = 'transport_logistics';
